...
char* indexes(const char* path)
{
    ...
    {
        char index_php[sizeof(path) + sizeof("/index.php") + 2];
        ...
        char index_html[sizeof(path) + sizeof("/index.html") + 2];
    ...
    */
    return NULL;
    /*char index_php[sizeof(path) + sizeof("/index.php")];
    strcpy(index_php, path);
    strcat(index_php, "/index.php");
    if ((path[strlen(path) - 1] != '/') && (access(index_php, F_OK) == 0))
    {
        return index_php;
    }
    else 
    ...
    char index_html[sizeof(path) + sizeof("/index.html")];
    strcpy(index_html, path);
    strcat(index_html, "/index.php");
    if ((index_html[strlen(index_html) - 1] != '/') && (access(index_html, F_OK) == 0))
    {
        return index_html;
    }
    else 
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // ensure content isn't already loaded
    if (*content != NULL)
    {
        return false;
    }
    ...
    *length = 0;
    while (true)
    {
        ...
        // if bytes were read, append to body
        if (bytes > 0)
        {
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    if ((spaceptr = strchr(line, ' ')) == NULL)
    {
        error(400);
        return false;
    }

    if ((httpptr = strstr(line, " HTTP")) == NULL)
    {
        error(400);
        return false;
    }
    
    if ((returnptr = strstr(line, "\r\n")) == NULL)
    {
        error(400);
        return false;
    }
    
    char *endchar = strchr(line, '\0');

    char methode[1 + lineLength];
    memset(methode, '\0', sizeof(line));
    strncpy(methode, line, spaceptr - line);
    
    char request_target[1 + lineLength];
    memset(request_target, '\0', sizeof(line));
    strncpy(request_target, 1 + spaceptr, httpptr - spaceptr - 1);

    char *quesptr = strchr(request_target, '?');
        
    char http_version[1 + lineLength];
    memset(http_version, '\0', sizeof(line));
    strncpy(http_version, httpptr+1, endchar - httpptr - 1);

    int absolute_pathLength = (sizeof(request_target) / sizeof(char));
    char absolute_path[sizeof(request_target) / sizeof(char)];
    memset(absolute_path, '\0', absolute_pathLength);
    
    if (quesptr == NULL)
    {
        strcpy(absolute_path, &request_target[0]);
    }
    else 
    {
        strncpy(absolute_path,&request_target[0],quesptr - &request_target[0]);
    }
    
    char *request_target_end = strchr(request_target,'\0');
    ...
    {
        query_length = request_target_end - quesptr;
    }
    ...
    if (query_length != 1)
    {
        strncpy(Query,quesptr+1,query_length-1);
    }
    
    for(int i = 0 , len = strlen(absolute_path); i < len; i++)
    {
        abs_path[i] = absolute_path[i];
    }
    
    for(int i = 0 , len = strlen(Query); i < len; i++)
    {
        query[i] = Query[i];
    }
    printf("%s\n", abs_path);
